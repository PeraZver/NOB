/*
 * Script: importDivisions.js
 * Description: Imports division data from a JSON file into the database.
...
const argv = yargs(hideBin(process.argv))
    .option('f', {
        alias: 'file',
        describe: 'Path to the JSON file containing division data',
        type: 'string',
        demandOption: true
    })
    .help()
...
const divisionsFilePath = path.resolve(argv.file); // Use the file path provided by the user
...
// Main function to import divisions
async function importDivisions() {
    let connection;

    try {
        // Read the JSON file
        const divisionsData = JSON.parse(fs.readFileSync(divisionsFilePath, 'utf8'));
        ...
        for (const division of divisionsData) {
            const { name, formation_date, formation_geo, formation_site, wikipedia_url } = division;

            // Fetch all existing division names and data from the database
            ...
            if (existingRows.length > 0) {
                ...
                if (updates.length > 0) {
                    const updateQuery = `UPDATE divisions SET ${updates.join(', ')} WHERE id = ?`;
            ...
            // Fetch all existing division names for similarity check
            const [allRows] = await connection.execute('SELECT name FROM divisions');
            ...
            if (bestMatch.bestMatch.rating > 0.99) {
                console.log(`Division "${name}" is very similar to "${bestMatch.bestMatch.target}". Skipping...`);
                continue;
            } else if (bestMatch.bestMatch.rating > 0.8) {
                const userResponse = await promptUser(
                    `Division "${name}" is somewhat similar to "${bestMatch.bestMatch.target}". Do you want to insert it? (yes/no): `
                );

                if (userResponse !== 'yes') {
                    console.log(`Division "${name}" was not inserted.`);
                    continue;
                }
            }
            ...
            const [idResult] = await connection.execute(
                'SELECT MAX(id) AS maxId FROM divisions'
            );
            ...
            await connection.execute(
                `INSERT INTO divisions (id, name, formation_date, formation_site, location, wikipedia_url)
                 VALUES (?, ?, ?, ?, ST_GeomFromText(?), ?)`,
                [nextId, name, safeFormationDate, safeFormationSite, location, safeWikipediaUrl]
            );

            console.log(`Division "${name}" has been inserted into the database.`);
        }
